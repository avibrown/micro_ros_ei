...
void ei_main(ei_interfaces__msg__EIResult* result_msg)
{
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
    	strcpy(classification.label, result.classification[ix].label);
    	classification.value = result.classification[ix].value;
    	
    	result_msg->result.push_back(classification);
    }
